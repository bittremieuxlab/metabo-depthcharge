...
def pad_stack(tensors, padding_amts):
    ...
    tensors = [
        ...
        for i, pad_len in zip(tensors, padding_amts, strict=False)
    ]
...
class JsonExtractor:
    ...
    def extract_json(
        ...
        # `json_obj` is the inner dict for a specific (formula, ion) pair,
        # i.e. json_file[form][ion] — caller is responsible for the lookup.
        ...
        if ablate_cls_error:
            ...
            cls_mass_diff = common.get_cls_mass_diff(parentmass, form=form, ion=ion)
...
class FormDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        actual_decoy_formulae = []
        actual_decoy_ions = []
        for decoy_formula, decoy_ion in zip(decoy_formulae, decoy_ions, strict=False):
            form_entry = json_object.get(decoy_formula)
            if form_entry is None or decoy_ion not in form_entry:
                logging.info(
                    f"Could not find ({decoy_formula}, {decoy_ion}) for {name}"
                )
                continue
            decoy_tbl = form_entry[decoy_ion]
            ...
            actual_decoy_formulae.append(decoy_formula)
            actual_decoy_ions.append(decoy_ion)
        ...
        cand_formulae = [true_formula] + actual_decoy_formulae
        cand_ions = [true_ion] + actual_decoy_ions
        cand_instruments = [instrument] * len(cand_ions)
        matched = [True] + [False] * len(actual_decoy_formulae)
        outdict = {
            "name": name,
            "matched": matched,
...
class PredDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for list_idx in self.new_ind_to_old_inds[idx]:
            ...
            form_entry = json_object.get(formula)
            if form_entry is None or ion not in form_entry:
                logging.info(f"Could not find ({formula}, {ion}) for {spec_name}")
                continue
            decoy_tbl = form_entry[ion]
            decoy_json = self.json_extractor.extract_json(
                json_obj=decoy_tbl,
                form=formula,
                ion=ion,
