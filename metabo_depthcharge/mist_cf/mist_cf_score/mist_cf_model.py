...
from .. import common, nn_utils
from . import mist_cf_data
...
class MistNet(pl.LightningModule):
    ...
    def _common_step(self, batch, name="train"):
        ...
        pred_inds = packed_out.argmax(-1)
        acc = (pred_inds == targ_inds).float().mean()
        output_loss = {"loss": nll_loss.mean()}

        self.log(f"{name}_loss", nll_loss.mean(), batch_size=len(targ_inds))
        self.log(f"{name}_acc", acc, batch_size=len(targ_inds))
        return output_loss
...
class FormulaTransformer(nn.Module):
    def __init__(
        ...
        ion_info=False,
        instrument_info=False,
        num_valid_ion=len(common.ION_LST),
        num_valid_instrument=common.max_instr_idx,
        **kwargs,
    ):
        ...
        # form embeddings + diff embeddings
        self.formula_dim = self.formula_dim_base * 2
        # cls_type flag, intensity, num_peaks, rel_mass_diff
        self.formula_dim += 4
    ...
    def forward(
        ...
        # Transformer: B x Np x d -> Np x B x d
